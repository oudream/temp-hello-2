...
bool CxSocket::ccid(cx::socket_t so, cx::uint8 ccid)
{
    cx::uint8 ccids[4];
...
int CxSocket::multicast(cx::socket_t so, unsigned ttl)
{
...
int CxSocket::wait(cx::timems_t timeout)
{
    bool mode = true;
...
int CxSocket::join(cx::socket_t so, const struct addrinfo *node)
{
    assert(node != nullptr);
...
int CxSocket::drop(cx::socket_t so, const struct addrinfo *node)
{
    assert(node != nullptr);
...
cx::socket_t CxSocket::create(const struct addrinfo *node, int stype, int sprotocol)
{
    assert(node != nullptr);
...
int CxSocket::error(cx::socket_t so)
{
...
int CxSocket::sendTimeout(cx::socket_t so, cx::msepoch_t to)
{
    assert(so != INVALID_SOCKET);
...
int CxSocket::receiveTimeout(cx::socket_t so, cx::msepoch_t to)
{
    assert(so != INVALID_SOCKET);
...
bool CxSocket::wait(cx::timems_t timeout) const
{
...
//don't support poll
bool CxSocket::waitSending(cx::timems_t timeout) const
{
    int status;
    struct timeval tv;
    struct timeval *tvp = &tv;
...
struct ::addrinfo *CxSocket::hinting(cx::socket_t so, struct addrinfo *hint)
{
    assert(hint != nullptr);
...
#endif
    so = INVALID_SOCKET;
    iowait = cx::LONG_MINUS_ONE;
    ioerr = 0;
}

CxSocket::CxSocket(cx::socket_t s)
{
#ifdef  _WIN32
    init();
#endif
...
cx::socket_t CxSocket::create(int family, int type, int protocol)
{
    cx::socket_t so;
#ifdef  _WIN32
    init();
...
void CxSocket::close(cx::socket_t so)
{
#ifdef  _WIN32
    ::closesocket(so);
#else
...
CxSocket &CxSocket::operator=(cx::socket_t s)
{
    release();
    so = s;
    return *this;
...
int CxSocket::family(cx::socket_t so)
{
    assert(so != INVALID_SOCKET);
...
std::string CxSocket::getSockAddress(cx::socket_t so)
{
    std::string r;
#ifdef _WIN32
...
std::string CxSocket::getPeerAddress(cx::socket_t so)
{
    std::string r;
#ifdef _WIN32
    struct sockaddr_storage addr;
...
void CxIpAddress::extractIpPort(const sockaddr *oSockAddr, std::string &sIp, cx::ushort &iPort)
{
    char sName[64];
    memset(sName, 0, sizeof(sName));
    CxSocket::query(oSockAddr, sName, sizeof(sName));
...
cx::uint32 CxIpAddress::ipV4ToInt32(const std::string &sIp)
{
    return ::inet_addr(sIp.c_str());
}

std::string CxIpAddress::ipV4ToString(cx::uint32 iIp)
{
    in_addr addr;
#ifdef _WIN32
...
void CxIpAddress::setPort(cx::ushort iPort)
{
...
bool CxIpAddress::isSameIpAddress(const std::string &sIp, cx::ushort iPort) const
{
...
static void freeaddrinfo(struct addrinfo *aip)
{
    while(aip != nullptr) {
        ...
        if(aip->ai_canonname != nullptr)
            free(aip->ai_canonname);
        if(aip->ai_addr != nullptr)
            free(aip->ai_addr);
        free(aip);
        aip = next;
    }
...
static int getnameinfo(const struct sockaddr *addr, socklen_t len, char *host, size_t hostlen, char *service, size_t servlen, int flags)
{
    ...
    struct servent *sp;
    assert(addr != nullptr);
    assert(host != nullptr || hostlen == 0);
    assert(service != nullptr || servlen == 0);
    ...
    if(hostlen > 0) {
        if(flags & NI_NUMERICHOST) {
            if(inet_ntop(addr->sa_family, addr, host, hostlen) == nullptr)
                return -1;
        }
        else {
            hostmutex.lock();
            hp = gethostbyaddr((cx::caddr_t)addr, len, addr->sa_family);
            if(hp != nullptr && hp->h_name != nullptr) {
                if(flags & NI_NOFQDN) {
                    cp = strchr(hp->h_name, '.');
                    if(cp)
                        *cp = 0;
    ...
    if(servlen > 0) {
        if(flags & NI_NUMERICSERV)
            ...
            servmutex.lock();
            sp = getservbyport(port, (flags & NI_DGRAM) ? "udp" : nullptr);
            if(sp && sp->s_name)
...
static int getaddrinfo(const char *hostname, const char *servname, const struct addrinfo *hintsp, struct addrinfo **res)
{
    ...
    struct addrinfo *aip = nullptr, *prior = nullptr;
    socklen_t len;
    short port = 0;
    ...
    *res = nullptr;
    ...
    for(np = hp->h_addr_list; *np != nullptr; np++) {
        aip = (struct addrinfo *)malloc(sizeof(struct addrinfo));
        memset(aip, 0, sizeof(struct addrinfo));
...
{
    assert(host != nullptr && *host != 0);
    assert(svc != nullptr && *svc != 0);
...
void CxIpAddressExtend::set(const char *host, unsigned port)
{
    ...
    if (port)
    {
        ...
        list = CxSocketExtend::query(host, nullptr);
}
...
cx::socket_t CxSocketExtend::create(const CxIpAddressExtend &address)
{
    cx::socket_t so;
...
socklen_t CxSocketExtend::query(cx::socket_t so, struct sockaddr_storage *sa, const char *host, const char *svc)
{
    assert(sa != nullptr);
    assert(host != nullptr && *host != 0);
    assert(svc != nullptr && *svc != 0);
...
int CxSocketExtend::bindto(cx::socket_t so, const char *host, const char *svc, int protocol)
{
    assert(so != INVALID_SOCKET);
    assert(host != nullptr && *host != 0);
    assert(svc != nullptr && *svc != 0);
...
cx::socket_t CxSocketExtend::create(const char *iface, const char *port, int family, int type, int protocol)
{
    assert(iface != nullptr && *iface != 0);
    assert(port != nullptr && *port != 0);
