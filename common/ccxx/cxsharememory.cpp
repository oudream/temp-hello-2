...
//  int prot = FILE_MAP_READ;
    ...
    if(len)
    {
        ...
        fd = CreateFileMapping(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0, len, fn);
    }
    else
...
remake:
    ...
    map = (cx::caddr_t)shmat(fd, nullptr, 0);
    if(!map)
        fault();
#ifdef  SHM_LOCK
    if(fd > -1)
        shmctl(fd, SHM_LOCK, nullptr);
#endif
}
...
bool CxShareMemory::create(const char *fn, size_t len)
{
    ...
    map = (cx::caddr_t)mmap(nullptr, len, prot, MAP_SHARED, fd, 0);
    if(!map)
        fault();
    ::close(fd);
    if(map != (cx::caddr_t)MAP_FAILED) {
        size = mapsize = len;
        mlock(map, mapsize);
#if INSERT_OFFSET > 0
        if(prot & PROT_WRITE) {
...
void CxShareMemory::fault() const
{
    abort();
}
