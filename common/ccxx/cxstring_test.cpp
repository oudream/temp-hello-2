...
TEST(CxString_ToX_HexPrefix, HexForms) {
    ...
    // with ok
    int v = CxString::toInt32("0x1A", &ok); EXPECT_TRUE(ok); EXPECT_EQ(v, 26);
    cx::uint u = CxString::toUint32("0x1A", &ok); EXPECT_TRUE(ok); EXPECT_EQ(u, (cx::uint)26);
    cx::int64 i64 = CxString::toInt64("0x1A", &ok); EXPECT_TRUE(ok); EXPECT_EQ(i64, (cx::int64)26);
    cx::uint64 u64 = CxString::toUint64("0x1A", &ok); EXPECT_TRUE(ok); EXPECT_EQ(u64, (cx::uint64)26);
}

TEST(CxString_ToX_WithOk, EdgeZeroAndInvalid) {
    bool ok = false;
    // zero branch behavior per implementation
    EXPECT_EQ(CxString::toInt32("0", &ok), 0); EXPECT_TRUE(ok);
    EXPECT_EQ(CxString::toUint32("0", &ok), (cx::uint)0); EXPECT_TRUE(ok);
    EXPECT_EQ(CxString::toInt64("0", &ok), (cx::int64)0); EXPECT_TRUE(ok);
    EXPECT_EQ(CxString::toUint64("0", &ok), (cx::uint64)0); EXPECT_TRUE(ok);

    double d = CxString::toDouble("0", &ok); EXPECT_EQ(d, 0.0); EXPECT_TRUE(ok);
    float f  = CxString::toFloat("0", &ok);  EXPECT_EQ(f, 0.0f); EXPECT_TRUE(ok);
    ...
    int i = CxString::fromString("abc", 123, &ok); EXPECT_FALSE(ok); EXPECT_EQ(i, 123);
    cx::uint32 uu = CxString::fromString("abc", (cx::uint32)88, &ok); EXPECT_FALSE(ok); EXPECT_EQ(uu, (cx::uint32)88);
    cx::int64 ii = CxString::fromString("abc", (cx::int64)77, &ok); EXPECT_FALSE(ok); EXPECT_EQ(ii, (cx::int64)77);
    cx::uint64 uu64 = CxString::fromString("abc", (cx::uint64)66, &ok); EXPECT_FALSE(ok); EXPECT_EQ(uu64, (cx::uint64)66);

    double dd = CxString::fromString("abc", 12.34, &ok); EXPECT_FALSE(ok); EXPECT_DOUBLE_EQ(dd, 12.34);
    float  ff = CxString::fromString("abc", 56.78f, &ok); EXPECT_FALSE(ok); EXPECT_FLOAT_EQ(ff, 56.78f);
}

TEST(CxString_SetFromString, AllTypes) {
    bool ok;
    bool b = false; ok = CxString::setFromString("1", b); EXPECT_TRUE(ok); EXPECT_TRUE(b);
    int i=0; ok = CxString::setFromString("0xFF", i); EXPECT_TRUE(ok); EXPECT_EQ(i,255);
    cx::uint32 u=0; ok = CxString::setFromString("0", u); EXPECT_TRUE(ok); EXPECT_EQ(u,(cx::uint32)0);
    cx::int64 i64=0; ok=CxString::setFromString("1234567890123", i64); EXPECT_TRUE(ok); EXPECT_EQ(i64,(cx::int64)1234567890123LL);
    cx::uint64 u64=0; ok=CxString::setFromString("0x10", u64); EXPECT_TRUE(ok); EXPECT_EQ(u64,(cx::uint64)16);
    double d=0; ok=CxString::setFromString("3.5", d); EXPECT_TRUE(ok); EXPECT_DOUBLE_EQ(d,3.5);
    float f=0; ok=CxString::setFromString("2.25", f); EXPECT_TRUE(ok); EXPECT_FLOAT_EQ(f,2.25f);
    string s; ok=CxString::setFromString("hello", s); EXPECT_TRUE(ok); EXPECT_EQ(s,"hello");

    // invalid integer -> false
    i=7; ok=CxString::setFromString("abc", i); EXPECT_FALSE(ok); EXPECT_EQ(i,7);
    // invalid float -> false
    f=1.0f; ok=CxString::setFromString("x.y", f); EXPECT_FALSE(ok); EXPECT_FLOAT_EQ(f,1.0f);
}
...
TEST(CxString_HexString_Data, Roundtrips) {
    ...
    bool ok=false;
    EXPECT_EQ(CxString::hexToInt32("FF", &ok), 255); EXPECT_TRUE(ok);
    EXPECT_EQ(CxString::hexToUint64("FFFFFFFFFFFFFFFF", &ok), (cx::uint64)0xFFFFFFFFFFFFFFFFULL); EXPECT_TRUE(ok);
...
TEST(CxString_Split_Family, CharStringCaseAndVec) {
    ...
    EXPECT_EQ(v1[0], "a"); EXPECT_EQ(v1[1], ""); EXPECT_EQ(v1[2], "b"); EXPECT_EQ(v1[3], "c");
    ...
    ASSERT_EQ(v3.size(), 4u); EXPECT_EQ(v3[2], "c");
...
TEST(CxString_Format_Token_Unquote, FormattingAndTokenizing) {
    ...
    EXPECT_TRUE(ok); EXPECT_EQ(t, "aaaaa"); EXPECT_EQ(s1, "12341234.bbbbbbbbb");

    s1 = "aaaaa.12341234.bbbbbbbbb";
    string tl = CxString::tokenLeft(s1, '.', &ok);
    EXPECT_TRUE(ok); EXPECT_EQ(tl, "12341234.bbbbbbbbb"); EXPECT_EQ(s1, "aaaaa");

    s1 = "aaaaa.12341234.bbbbbbbbb";
    string tr = CxString::tokenRight(s1, '.', &ok);
    EXPECT_TRUE(ok); EXPECT_EQ(tr, "bbbbbbbbb"); EXPECT_EQ(s1, "aaaaa.12341234");
