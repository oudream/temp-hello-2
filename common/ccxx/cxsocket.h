...
public:
     ...
     * @param addr : sockaddr_storage or sockaddr , address storage for local address.
     * @return 0 on success, -1 on failure.
     */
    static int local(cx::socket_t socket, struct sockaddr_storage *addr);
     ...
     * the future.
     * @param socket descriptor to examine.
     * @param addr : sockaddr_storage or sockaddr , address storage for remote address.
     * @return 0 on success, -1 on failure.
     */
    static int remote(cx::socket_t socket, struct sockaddr_storage *addr);
     ...
     * @return
     */
    static std::string getSockAddress(cx::socket_t so);
     ...
     * @return
     */
    static std::string getPeerAddress(cx::socket_t so);
     ...
     * @return socket type.
     */
    static int type(cx::socket_t socket);
     ...
     */
    static bool ccid(cx::socket_t socket, cx::uint8 id);
     ...
     */
    static unsigned pending(cx::socket_t socket);
     ...
     */
    static int receiveTimeout(cx::socket_t so, cx::msepoch_t to);
     ...
     * @return 0 on success, -1 on error.
     */
    static int sendTimeout(cx::socket_t so, cx::msepoch_t to);
     ...
     * @return 0 on success, -1 on error.
     */
    static int disconnect(cx::socket_t socket);
     ...
     * @return 0 on success, -1 on error.
     */
    static int drop(cx::socket_t socket, const struct addrinfo *list);
     ...
     * @return 0 on success, -1 on error.
     */
    static int join(cx::socket_t socket, const struct addrinfo *list);
     ...
     */
    static int error(cx::socket_t socket);
     ...
     */
    static int multicast(cx::socket_t socket, unsigned ttl = 1);
     ...
     * @return 0 if success, -1 if error.
     */
    static int blocking(cx::socket_t socket, bool enable);
     ...
     * @return 0 if success, -1 if error.
     */
    static int broadcast(cx::socket_t socket, bool enable);
     ...
     * @return 0 if success, -1 if error.
     */
    static int nodelay(cx::socket_t socket);
     ...
     * @return 0 on success, -1 on error.
     */
    static int priority(cx::socket_t socket, int scheduling);
     ...
     * @return 0 on success, -1 on error.
     */
    static int tos(cx::socket_t socket, int type);
     ...
     * @return 0 on success, -1 on error.
     */
    static int ttl(cx::socket_t socket, unsigned char time);

    /**
     * Get the address family of the socket descriptor.
     * @return address family.
     */
    static int family(cx::socket_t socket);
     ...
     */
    static cx::socket_t create(int family, int type, int protocol);
     ...
     * @return socket descriptor created or INVALID_SOCKET.
     */
    static cx::socket_t create(const struct addrinfo *addr, int type, int protocol);
     ...
     * @return hint buffer.
     */
    static struct addrinfo *hinting(cx::socket_t socket, struct addrinfo *hint);
     ...
     * @return buffer if found or nullptr if not.
     */
     ...
     * @return void.
     */
    static void close(cx::socket_t so);
...
public:
     ...
     * @return true if input data waiting.
     */
    bool wait(cx::timems_t timeout = 0) const;
     ...
     */
    bool waitSending(cx::timems_t timeout = 0) const;
     ...
     */
    int wait(cx::timems_t timeout = cx::LONG_MINUS_ONE);
     ...
     * future there will be a single puts() that has a nullptr default.
 ...
 * @author David Sugar <dyfet@gnutelephony.org>
 */
class CCXX_EXPORT CxIpAddressExtend
{
protected:
...
public:
     ...
     * @param service port or name we are referencing or nullptr.
     */
    CxIpAddressExtend(int family, const char *hostname, const char *service = nullptr);
     ...
     * @return first socket address or nullptr if none.
     */
     ...
     * @return first socket address of family or nullptr if none.
     */
     ...
     * @return matching address from list or nullptr if not found.
     */
     ...
     * @param service name or port number, or nullptr if not used.
     * @param type of socket (stream or dgram) to filter list by.
     */
    void set(const char *hostname, const char *service = nullptr, int type = SOCK_STREAM);
     ...
     */
    void add(const char *hostname, const char *service = nullptr, int type = SOCK_STREAM);
     ...
     * @param address to convert.
     * @return new ipv4 address or nullptr if not ipv4.
     */
...
public:
     ...
     * @return buffer or nullptr if lookup fails.
     */
     ...
     */
    static socklen_t query(cx::socket_t socket, struct sockaddr_storage *addr, const char *hostname, const char *service);
     ...
     * @return 0 on success, -1 if error.
     */
    static int bindto(cx::socket_t socket, const char *addr, const char *service, int protocol = 0);
     ...
     * @return socket descriptor created or INVALID_SOCKET.
     */
    static cx::socket_t create(const char *iface, const char *service, int family = AF_UNSPEC, int type = 0, int protocol = 0);
     ...
     */
    static cx::socket_t create(const CxIpAddressExtend &IpAddress);
 ...
 * @author David Sugar <dyfet@gnutelephony.org>
 */
class CCXX_EXPORT CxListenSocket : protected CxSocketExtend
{
public:
    /**
     ...
     */
    static cx::socket_t create(const char *addr, const char *service, unsigned backlog = 5, int family = AF_UNSPEC, int type = 0, int protocol = 0);
     ...
     */
    cx::socket_t accept(struct sockaddr_storage *addr = nullptr) const;
