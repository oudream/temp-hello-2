...
const cx::uchar f_crc8Table[256] = {
...
const cx::ushort f_crc16Table[256] =     //crc16 table
        {
...
const cx::uchar f_modbusTableHi[256] = {
...
const cx::uchar f_modbusTableLo[256] = {
...
cx::uchar CxCrc::calcCRC8(const cx::uchar *pData, int iLength)
{
    cx::uchar result = 0;
    int i = 0;
    for (; i < iLength; ++i)
...
cx::ushort CxCrc::calcCRC16(const cx::uchar *pData, int iLength)
{
    cx::ushort result = 0;
    int i = 0;
    for (; i < iLength; ++i)
...
{
    const cx::uchar *pData1 = (const cx::uchar *) pData;
    cx::ushort result = 0;
    int i = 0;
    for (; i < iLength; ++i)
    {
...
cx::ushort CxCrc::calcModbus(const cx::uchar *pData, int iLength)
{
    cx::uchar hi = 0xFF;
    cx::uchar lo = 0xFF;
    cx::uchar index;
...
#define SET(n) \
    (ctx->block[(n)] = \
    (cx::uint32)ptr[(n) * 4] | \
    ((cx::uint32)ptr[(n) * 4 + 1] << 8) | \
    ((cx::uint32)ptr[(n) * 4 + 2] << 16) | \
    ((cx::uint32)ptr[(n) * 4 + 3] << 24))
#define GET(n) \
    (ctx->block[(n)])
#endif
...
static const unsigned char *body(struct md4_context *ctx, const unsigned char *data, size_t size)
{
    ...
    cx::uint32 a, b, c, d;
    cx::uint32 saved_a, saved_b, saved_c, saved_d;
...
{
    const cx::uint32 byteOrderTest = 0x1;
...
static inline cx::uint32 rol32(cx::uint32 value, unsigned int shift)
{
#ifdef GM_CC_MSVC
...
static inline cx::uint32 sha1Word(Sha1Chunk *chunk, const cx::uint32 position)
{
    return (
            chunk->words[position & 0xf] = rol32(chunk->words[(position + 13) & 0xf]
                                                 ^ chunk->words[(position + 8) & 0xf]
                                                 ^ chunk->words[(position + 2) & 0xf]
                                                 ^ chunk->words[(position) & 0xf], 1));
}

static inline void sha1Round0(Sha1Chunk *chunk, const cx::uint32 position,
                              cx::uint32 &v, cx::uint32 &w, cx::uint32 &x, cx::uint32 &y, cx::uint32 &z)
{
    z += (((w & (x ^ y)) ^ y) + chunk->words[position] + 0x5A827999 + rol32(v, 5));
    w = rol32(w, 30);
}

static inline void sha1Round1(Sha1Chunk *chunk, const cx::uint32 position,
                              cx::uint32 &v, cx::uint32 &w, cx::uint32 &x, cx::uint32 &y, cx::uint32 &z)
{
...
static inline void sha1Round2(Sha1Chunk *chunk, const cx::uint32 position,
                              cx::uint32 &v, cx::uint32 &w, cx::uint32 &x, cx::uint32 &y, cx::uint32 &z)
{
...
static inline void sha1Round3(Sha1Chunk *chunk, const cx::uint32 position,
                              cx::uint32 &v, cx::uint32 &w, cx::uint32 &x, cx::uint32 &y, cx::uint32 &z)
{
...
static inline void sha1Round4(Sha1Chunk *chunk, const cx::uint32 position,
                              cx::uint32 &v, cx::uint32 &w, cx::uint32 &x, cx::uint32 &y, cx::uint32 &z)
{
...
#include "cxcrc_endian.cpp"

static inline void sha1ProcessChunk(Sha1State *state, const unsigned char *buffer)
{
    // Copy state[] to working vars
    cx::uint32 a = state->h0;
    cx::uint32 b = state->h1;
    cx::uint32 c = state->h2;
    cx::uint32 d = state->h3;
    cx::uint32 e = state->h4;

    cx::uint8 chunkBuffer[64];
...
static inline void sha1Update(Sha1State *state, const unsigned char *data, cx::int64 len)
{
    cx::uint32 rest = static_cast<cx::uint32>(state->messageSize & GM_UINT64_C(63));

    cx::uint64 availableData = static_cast<cx::uint64>(len) + static_cast<cx::uint64>(rest);
    ...
    {
        cx::int64 i = static_cast<cx::int64>(64 - rest);
        memcpy(&state->buffer[rest], &data[0], static_cast<cx::int32>(i));
        sha1ProcessChunk(state, state->buffer);

        cx::int64 lastI = len - ((len + rest) & GM_INT64_C(63));
        for (; i < lastI; i += 64)
...
{
    cx::uint64 messageSize = state->messageSize;
...
string CxCrc::file2md5(const string &sFilePath, int iCodeType)
{
    ...
    if (!CxFilesystem::readAllBytes(sFilePath, fileData))
    {
        return r;
    }
...
{
    auto pathInfo1 = CxFilesystem::stat(sFilePath1);
    auto pathInfo2 = CxFilesystem::stat(sFilePath2);
    if (pathInfo1.size > 0)
    {
...
cx::uchar CxCrc::calcXOR(const cx::uchar *pData, int iLength)
{
    cx::uchar Res = pData[0];
...
cx::uchar CxCrc::crc8(const cx::uchar *ptr, int len)
{
    cx::uchar crc = 0;
    int i = 0;
