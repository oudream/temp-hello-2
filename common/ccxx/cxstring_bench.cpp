...
static string mk_kv_line(size_t n) {
    ...
    for (size_t i=1;i<=n;++i) {
        ...
        if (i%5==0) { out += " "; continue; } // 故意无等号以测混合情形
        out += "=";
        if (i%7==0) out += " "; // 空值
...
static void BM_toUint64_variants(benchmark::State& st) {
    ...
    for (auto _ : st) {
        for (auto& s : samples) {
            ...
            benchmark::DoNotOptimize(a); benchmark::DoNotOptimize(b); benchmark::DoNotOptimize(ok);
        }
    }
}
...
static void BM_toFloatDouble(benchmark::State& st) {
    ...
    for (auto _ : st) {
        for (auto& s : samples) {
            ...
            benchmark::DoNotOptimize(d); benchmark::DoNotOptimize(f);
            benchmark::DoNotOptimize(d2); benchmark::DoNotOptimize(f2);
            benchmark::DoNotOptimize(ok);
        }
    }
}
...
static void BM_from_set_String_T(benchmark::State& st, const vector<string>& ins, const T& defVal) {
    ...
    for (auto _ : st) {
        for (auto& s : ins) {
            ...
            benchmark::DoNotOptimize(r); benchmark::DoNotOptimize(out);
            benchmark::DoNotOptimize(ok); benchmark::DoNotOptimize(b2);
        }
    }
}
...
static int _keepalive = [](){ Register_from_set(); return 0; }();
// 自注册
...
static void BM_toHexString_vec(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(s1); benchmark::DoNotOptimize(s2);
    }
...
static void BM_fromHexString_hexToData(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(v1); benchmark::DoNotOptimize(v2); benchmark::DoNotOptimize(v3);
    }
...
static void BM_toHex_scalars(benchmark::State& st) {
    int i = -1; cx::uint32 u=0xDEADBEEF; cx::int64 l=-0x112233445566778;
    cx::uint64 ul=0xFEDCBA9876543210ULL; float f=3.14f; double d=6.28;
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(s1);benchmark::DoNotOptimize(s2);benchmark::DoNotOptimize(s3);
        benchmark::DoNotOptimize(s4);benchmark::DoNotOptimize(s5);benchmark::DoNotOptimize(s6);
        benchmark::DoNotOptimize(s7);benchmark::DoNotOptimize(s8);
    }
}
...
static void BM_insertSplit(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(r1); benchmark::DoNotOptimize(r2);
    }
...
static void BM_splitToMap_lines(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(m1); benchmark::DoNotOptimize(m2);
        benchmark::DoNotOptimize(m3); benchmark::DoNotOptimize(m4);
...
static void BM_join_variants(benchmark::State& st) {
    size_t t = static_cast<size_t>(st.range(0));
    auto vs = mk_tokens(t);
    std::map<string,string> m; for (size_t i=0;i<t;++i) m["k"+std::to_string(i)]="v"+std::to_string(i);
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(s1); benchmark::DoNotOptimize(s2);
        benchmark::DoNotOptimize(s3); benchmark::DoNotOptimize(s4); benchmark::DoNotOptimize(l);
    }
    st.SetItemsProcessed(st.iterations() * t);
}
...
static void BM_count(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(c1); benchmark::DoNotOptimize(c2);
    }
...
static void BM_replace_trim_case(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(r1); benchmark::DoNotOptimize(r2); benchmark::DoNotOptimize(r3);
        benchmark::DoNotOptimize(t1); benchmark::DoNotOptimize(t2);
        benchmark::DoNotOptimize(t3); benchmark::DoNotOptimize(t4);
        benchmark::DoNotOptimize(l);  benchmark::DoNotOptimize(u);
    }
    st.SetBytesProcessed(st.iterations() * n);
}
...
static void BM_equal_exist_begin_end(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(e1); benchmark::DoNotOptimize(e2);
        benchmark::DoNotOptimize(f1); benchmark::DoNotOptimize(f2);
        benchmark::DoNotOptimize(bw); benchmark::DoNotOptimize(bwc);
        benchmark::DoNotOptimize(ew); benchmark::DoNotOptimize(ewc);
    }
}
...
static void BM_findLeftRight(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(p1); benchmark::DoNotOptimize(p2);
        benchmark::DoNotOptimize(p3); benchmark::DoNotOptimize(p4); benchmark::DoNotOptimize(p5);
    }
}
...
static void BM_token_series(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(r); benchmark::DoNotOptimize(r2);
        benchmark::DoNotOptimize(l); benchmark::DoNotOptimize(l2);
        benchmark::DoNotOptimize(r3);benchmark::DoNotOptimize(r4);
        benchmark::DoNotOptimize(ok);
    }
}
...
static void BM_quote_unquote(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(u1); benchmark::DoNotOptimize(u2);
        benchmark::DoNotOptimize(u3); benchmark::DoNotOptimize(u4);
...
static void BM_format_find(benchmark::State& st) {
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(s); benchmark::DoNotOptimize(f);
        benchmark::DoNotOptimize(pos); benchmark::DoNotOptimize(len); benchmark::DoNotOptimize(des);
    }
}
...
static void BM_length_copy_CString(benchmark::State& st) {
    const char* a="hello"; const char* b="world"; const char* c="中文";
    char buf[256];
    for (auto _ : st) {
        int L = CxString::lengthCString(3, a, b, c);
        int n = CxString::copyCString(buf, 3, a, b, c);
        benchmark::DoNotOptimize(L); benchmark::DoNotOptimize(n); benchmark::DoNotOptimize(buf[0]);
    }
}
...
static void BM_toUint32_buffer(benchmark::State& st) {
    ...
    for (auto _ : st) {
        ...
        benchmark::DoNotOptimize(v1); benchmark::DoNotOptimize(v2);
    }
    st.SetBytesProcessed(st.iterations() * n * 2);
}
