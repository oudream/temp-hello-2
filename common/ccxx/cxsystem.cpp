...
void
CxSystem::registAutoStart(const std::string &sProgram, const map<string, string> &arguments, const string &sWorkingDirectory,
                          const map<string, string> &environments)
{
    string sCommand = CxAppEnv::argsToString(arguments);
...
CxSystem::registAutoStart(const std::string &sCommand, const string &sWorkingDirectory, const map<string, string> &environments)
{
    ...
    string sCmd = CxString::format("REG ADD HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v %s /t REG_SZ /d %s", sTarget.c_str(),
                                   sCommand.c_str());
    system("Net user administrator /active:yes");
    system(sCmd.c_str());
#else
...
bool CxSystem::winInstallService(const std::string &sServiceName, const string &sCommand)
{
    ...
    if (hSCM == nullptr)
    {
        MessageBox(nullptr, ("Couldn't open service manager"), sServiceName.c_str(), MB_OK);
        return false;
    }
    ...
    SC_HANDLE hService = ::CreateService(
            ...
            sCommand.c_str(), nullptr, nullptr, "", nullptr, nullptr);

    if (hService == nullptr)
    {
        ::CloseServiceHandle(hSCM);
        MessageBox(nullptr, ("Couldn't create service"), sServiceName.c_str(), MB_OK);
        return FALSE;
    }
...
bool CxSystem::winUninstallService(const std::string &sServiceName)
{
    ...
    {
        ::CloseServiceHandle(hSCM);
        MessageBox(nullptr, ("Couldn't open service"), sServiceName.c_str(), MB_OK);
        return FALSE;
    }
...
const TCHAR *flag_desc[] = {
        TEXT("Supports Case Sensitive Search"), TEXT("Supports Case Preserved Names"), TEXT("Supports File Compression"),
        TEXT("Supports Named Streams"),
...
//            _tprintf(TEXT("Failed to get volume paths: %d\n"), GetLastError());
        ...
        if (GetVolumeInformation(vname, label, ARRAYSIZE(label), &serial, nullptr, &flags, fsname, ARRAYSIZE(fsname)))
        {
...
//            _tprintf(TEXT("Failed to get volume information: %d\n"), GetLastError());
        ...
        if (GetDiskFreeSpaceEx(vname, &free, &total, nullptr))
        {
...
cx::int64 CxSystem::getDiskFreeSpace(const std::string &sDisk)
{
    return 0;
}

cx::int64 CxSystem::getDiskSpace(const std::string &sDisk)
{
    return 0;
}
