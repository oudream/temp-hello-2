...
protected:
    void run()
    {
        ...
        CxProcess::threadEventNotify(nullptr, GM_EVENT_PROCESS_END, 0, (void *) &m_pid, sizeof(m_pid), nullptr);
#endif
    }
                         ...
                         const map<string, string> &environments)
{
    ...
    PVOID pEnvs = nullptr;
    LPCSTR pDir = (sWorkingDirectory.size() > 0) ? sWorkingDirectory.c_str() : nullptr;
    ...
    pEnvs = ((environmentArray.size() > 0)) ? (&environmentArray.front()) : nullptr;
...
//    }
    BOOL result = ::CreateProcessAsUser(primaryToken, 0, (LPSTR) (sCommand.c_str()), nullptr, nullptr, FALSE,
                                        CREATE_NEW_CONSOLE | NORMAL_PRIORITY_CLASS
                                        | CREATE_UNICODE_ENVIRONMENT, nullptr, 0, &StartupInfo, &r/*&processInfo*/);
...
cx::pid_os_t CxProcess::allocEmpty()
{
    cx::pid_os_t r;
    ZeroMemory(&r, sizeof(r));
    return r;
...
bool CxProcess::isvalid(cx::pid_os_t pid)
{
    return pid.dwProcessId != 0 && pid.dwThreadId != 0 && pid.hProcess != nullptr && pid.hThread != nullptr;
}

bool CxProcess::isSame(cx::pid_os_t pid1, cx::pid_os_t pid2)
{
    return pid1.hProcess == pid2.hProcess;
}

bool CxProcess::kill(cx::pid_os_t pid)
{
    return TerminateProcess(pid.hProcess, 0);
}

void CxProcess::wait(cx::pid_os_t pid)
{
    WaitForSingleObject(pid.hProcess, INFINITE);
}

void CxProcess::release(cx::pid_os_t pid)
{
...
{
    string argv0;
    map<string, string> arguments = CxArguments::parseCommandLine(sArguments, argv0);
                         ...
                         const map<string, string> & environments)
{
    ...
    else if (pid == 0)
    {
        ...
        char * * argv = ((argumentArray.size()>0)) ? (& argumentArray.front()) : nullptr;
        //environment
        vector<string> environmentList = CxString::joinToStringList(environments, "=");
        vector<char *> environmentArray = fn_joinPcharList(environmentList);
        char * * envp = ((environmentArray.size()>0)) ? (& environmentArray.front()) : nullptr;
...
cx::pid_os_t CxProcess::execAsDetached(const string & sProgram,
                                   const map<string, string> & arguments,
                                   const string & sWorkingDirectory,
                                   const map<string, string> & environments)
{
...
cx::pid_os_t CxProcess::getCurrentPid()
{
    cx::pid_os_t r;
#ifdef _WIN32
...
void CxProcess::dealThreadNofity(int iEvent, int iTag, const void *pData, int iLength, void *oSource, void *oTarget)
{
    if (iEvent == GM_EVENT_PROCESS_END)
    {
        ...
        ProcessWaitThread *oProcessWaitThread = nullptr;
...
bool CxProcess::isRunning(cx::pid_os_t pid)
{
    bool r = false;
#ifdef _WIN32
    HANDLE hSnapShot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
...
int CxProcess::getMemoryUsage(cx::pid_os_t pid)
{
#ifdef _WIN32
...
cx::nsepoch_t CxProcess::getCpuUsage(cx::pid_os_t pid, int *pUsage)
{
    cx::nsepoch_t r = 0;
#ifdef _WIN32
...
cx::nsepoch_t CxProcess::getCpuUsage()
{
    cx::nsepoch_t r = 0;
#ifdef _WIN32
