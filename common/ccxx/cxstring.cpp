...
string CxString::toString(const cx::uint32 &n)
{
    char str[10 + 1];
    sprintf(str, "%u", n);
    return string(str);
...
string CxString::toString(const cx::ulong &n)
{
    stringstream ss;
    ss << n;
    return ss.str();
...
string CxString::toString(const cx::int64 &n)
{
    stringstream ss;
    ss << n;
    return ss.str();
...
string CxString::toString(const cx::uint64 &n)
{
    stringstream ss;
    ss << n;
    return ss.str();
...
cx::uint CxString::toUint32(const string &sSource)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::uint n;
...
cx::int64 CxString::toInt64(const string &sSource)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::int64 n;
...
cx::uint64 CxString::toUint64(const string &sSource)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::uint64 n;
...
{
    return strtod(sSource.data(), nullptr);
}
...
cx::uint CxString::toUint32(const string &sSource, bool *bOk)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::uint n;
...
cx::int64 CxString::toInt64(const string &sSource, bool *bOk)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::int64 n;
...
cx::uint64 CxString::toUint64(const string &sSource, bool *bOk)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::uint64 n;
...
{
    double d = strtod(sSource.data(), nullptr);
    if (d != 0)
    {
        if (bOk) *bOk = true;
        return d;
...
{
    float d = strtod(sSource.data(), nullptr);
    if (d != 0)
    {
        if (bOk) *bOk = true;
        return d;
...
cx::int8 CxString::fromString(const string &sSource, const cx::int8 &i, bool *bOk)
{
    int v = i;
    return fromString(sSource, v, bOk);
}

cx::uint8 CxString::fromString(const string &sSource, const cx::uint8 &i, bool *bOk)
{
    int v = i;
    return fromString(sSource, v, bOk);
}

cx::int16 CxString::fromString(const string &sSource, const cx::int16 &i, bool *bOk)
{
    int v = i;
    return fromString(sSource, v, bOk);
}

cx::uint16 CxString::fromString(const string &sSource, const cx::uint16 &i, bool *bOk)
{
    int v = i;
    return fromString(sSource, v, bOk);
}
...
cx::uint32 CxString::fromString(const string &sSource, const cx::uint32 &n, bool *bOk)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::uint32 r;
...
cx::int64 CxString::fromString(const string &sSource, const cx::int64 &n, bool *bOk)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::int64 r;
...
cx::uint64 CxString::fromString(const string &sSource, const cx::uint64 &n, bool *bOk)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::uint64 r;
...
{
    double r = strtod(sSource.data(), nullptr);
    if (r != 0)
    {
        if (bOk) *bOk = true;
        return r;
...
{
    float r = strtod(sSource.data(), nullptr);
    if (r != 0)
    {
        if (bOk) *bOk = true;
        return r;
...
bool CxString::setFromString(const string &sSource, cx::uint32 &n)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        sscanf(sSource.data(), "%x", &n);
...
bool CxString::setFromString(const string &sSource, cx::int64 &n)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::int64 r;
...
bool CxString::setFromString(const string &sSource, cx::uint64 &n)
{
    if (sSource.size() > 2 && (sSource[1] == 'x' || sSource[1] == 'X'))
    {
        cx::uint64 r;
...
string CxString::toHexString(const cx::uchar *pData, int iLength, bool bHasEmptyChar)
{
    if (bHasEmptyChar)
    {
        string sHex(iLength * 3, 0);
...
string CxString::toHexString(const vector<cx::uchar> &data, bool bHasEmptyChar)
{
    if (data.size() > 0)
        return toHexString((&data.front()), data.size(), bHasEmptyChar);
    else
...
string CxString::toHexString(const cx::uchar &v)
{
    string s = "00";
    char *data = const_cast<char *>( s.data());
    int j = (v >> 4) & 0xf;
...
string CxString::toHexString(const cx::ushort &v)
{
    string s = "0000";
    char *data = const_cast<char *>( s.data());
    int j = (v >> 12) & 0xf;
...
string CxString::toHexString(const cx::uint32 &v)
{
    string s = "00000000";
    char *data = const_cast<char *>( s.data());
    int j = (v >> 28) & 0xf;
...
string CxString::toHexString(const cx::int64 &v)
{
...
string CxString::toHexString(const cx::uint64 &v)
{
    string s = "0000000000000000";
    char *data = const_cast<char *>( s.data());
    cx::int64 j = (v >> 60) & 0xf;
...
string CxString::toHexString(const cx::uint32 &v, bool bHasHead)
{
    string s = "00000000";
    char *data = const_cast<char *>( s.data());
    int j = (v >> 28) & 0xf;
...
std::vector<cx::uchar> CxString::fromHexString(const string &s)
{
    return f_fromHexString<cx::uchar>(s);
}
...
cx::uint CxString::hexToUint32(const string &s, bool *bOk)
{
    cx::uint r;
    int iCount = sscanf(s.data(), "%x", &r);
    if (bOk) *bOk = (iCount == 1);
...
cx::int64 CxString::hexToInt64(const string &s, bool *bOk)
{
    cx::int64 r;
...
cx::uint64 CxString::hexToUint64(const string &s, bool *bOk)
{
    cx::uint64 r;
    int iCount = sscanf(s.data(), "%llx", &r);
    if (bOk) *bOk = (iCount == 1);
...
map<string, string> CxString::splitToMap(const vector<string> &ss, char cMid, bool bTrim)
{
    ...
    if (bTrim)
    {
        for (size_t i = 0; i < ss.size(); ++i)
        {
            ...
            size_t endSplit = sContent.find(cMid);
    ...
    while (1)
    {
        ...
        if (endMid != string::npos)
        {
            ...
            if (endSplit != string::npos)
            {
                ...
                sValue = trim(sValue, cx::CHAR_SPACE);
                start = endSplit + 1;
                r[sValue] = sKey;
            }
            else
...
string CxString::replace(const string &strBase, char cSrc, char cDes)
{
    ...
    cx::uchar *first = (cx::uchar *) r.data();
    cx::uchar *end = first + r.size();
...
{
    if (s.empty()) return s;
    cx::uchar *first = (cx::uchar *) s.data();
    cx::uchar *end = first + s.size() - 1;
    cx::uchar *pchFirst = first;
    cx::uchar *pchEnd = end;
    if (s.size() == 1)
    {
        if (*first < '!')
...
{
    if (s.empty()) return s;
    cx::uchar cDelete2 = cx::uchar(cDelete);
    cx::uchar *first = (cx::uchar *) s.data();
    cx::uchar *end = first + s.size() - 1;
    cx::uchar *pchFirst = first;
    cx::uchar *pchEnd = end;
...
vector<string> CxString::unquotes(const string &s, const string &quote_l, const string &quote_r)
{
    vector<string> r;
    if (quote_l.empty() || quote_r.empty())
    {
    ...
    size_t iIndexLeft = s.find(quote_l);
    while (iIndexLeft != string::npos)
    {
        size_t iPosLeft = iIndexLeft + quote_l.size();
        size_t iIndexRight = s.find(quote_r, iPosLeft);
        if (iIndexRight != string::npos)
        {
            string str = s.substr(iPosLeft, iIndexRight - iPosLeft);
            r.push_back(str);
        }
        else
        {
        ...
        iIndexLeft = s.find(quote_l, iIndexRight);
    }
    return r;
}
...
cx::uint32 CxString::toUint32(char *pBuff, int iLenth, bool bInverse)
{
    cx::uint32 val = 0;
    if (bInverse)
    {
...
/*
cx::uint32 CxString::toUint32(std::string szBuf)
{
    cx::uint32 val = 0;
    string tmp="";
    size_t i=0;
    ...
    if((tmp.substr(0,2)=="0x")||(tmp.substr(0,2)=="0X"))
    {
        ...
        vector<cx::uchar> v = fromHexString(tmp);
...
*/
//std::string CxString::ipToString(cx::uint32 val)
//{
//    char*pBuf = (char*)&val;
//    char cTmp[20];
...
//cx::uint32 CxString::ipToUint32(std::string val)
//{
//    //needtodo
//    return 0;
//}
