...
#include "project//u_project_page.h"
#include "acquisition//u_assets_page.h"
#include "reconstruction//u_reconstruct_page.h"
#include "mc/u_scene_page.h"
#include "inspection/u_inspect_page.h"

// Panels
#include "panels/u_panel_base.h"
#include "panels/u_panel_assets.h"
#include "panels/u_panel_camera.h"
#include "panels/u_panel_knife.h"
#include "panels/u_panel_render.h"
#include "panels/u_panel_measure.h"
#include "panels/u_panel_log.h"

// 状态栏
#include "u_status_bar.h"

static const char *K_LAST_PAGE = "last_page";
static const char *K_LAYOUT_MODE = "layout_mode";
static const char *K_PANEL_VIS = "panel/%1/visible";
...
{
    // 3D 宿主
    _pageMgr->registerPage(AppIds::RibbonPageToLabel(AppIds::ERibbonPage::Project), new UProjectPage(this));
    _pageMgr->registerPage(AppIds::RibbonPageToLabel(AppIds::ERibbonPage::Acquisition), new UAssetsPage(this));
    _pageMgr->registerPage(AppIds::RibbonPageToLabel(AppIds::ERibbonPage::Reconstruction), new UReconstructPage(this));
    _pageMgr->registerPage(AppIds::RibbonPageToLabel(AppIds::ERibbonPage::Editor), new MScenePage(this));
    _pageMgr->registerPage(AppIds::RibbonPageToLabel(AppIds::ERibbonPage::Inspection), new UInspectPage(this));
...
void MctMainWindow::registerPanels()
{
    ...
    auto *pnlAssets = new UPanelAssets(this);
...
void MctMainWindow::setupShortcuts()
{
    ...
    mk(QKeySequence("Ctrl+1"), [this]
    { onRequestSwitchPage(0); });
    mk(QKeySequence("Ctrl+2"), [this]
    { onRequestSwitchPage(1); });
    mk(QKeySequence("Ctrl+3"), [this]
    { onRequestSwitchPage(2); });
    mk(QKeySequence("Ctrl+4"), [this]
    { onRequestSwitchPage(3); });
    mk(QKeySequence("Ctrl+5"), [this]
    { onRequestSwitchPage(4); });
    mk(QKeySequence("Ctrl+G"), [this]
    { onRequestLayoutGrid(); });
    mk(QKeySequence("Ctrl+J"), [this]
    { onRequestLayout13(); });
    mk(QKeySequence("Ctrl+E"), [this]
    { onPageRequestLayoutChange("equalize"); });
}

// --- Ribbon 请求 ---

void MctMainWindow::onRequestSwitchPage(int pageIndex)
{
    _pageMgr->switchTo(pageIndex);
}

void MctMainWindow::onRequestLayoutGrid()
{
    // 转发到 3D 宿主
    if (auto *page = _pageMgr->pageTo<MScenePage>())
    {
        // 2×2 布局（需求规格 6）           :contentReference[oaicite:15]{index=15}
        page->reLayout("grid_2x2");
...
void MctMainWindow::showEvent(QShowEvent *e)
{
    ...
    if (auto *page = _pageMgr->pageTo<MScenePage>()) page->reLayout(_currentLayout);
